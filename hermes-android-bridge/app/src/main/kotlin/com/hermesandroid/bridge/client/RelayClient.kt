...
object RelayClient {
    ...
    private const val MAX_RETRIES = 999999 // CUBA SAMPAI KIAMAT (SOVEREIGN)
    ...
    /** SOVEREIGN: Send proactive events to the AI Agent */
    fun sendEvent(eventType: String, data: Any) {
        ...
        scope?.launch {
            try {
                ...
                ws.send(eventJson.toString())
                if (BuildConfig.DEBUG) Log.d(TAG, "Sent Sovereign Event: $eventType")
            } catch (e: Exception) {
                Log.e(TAG, "Failed to send Sovereign Event", e)
            }
        }
    }
